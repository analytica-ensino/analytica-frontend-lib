...
describe('StudentLessonProgressModal', () => {
  ...
  describe('Nested Accordion Behavior', () => {
    ...
    // The collapsed body stays mounted: it must be out of the accessibility
    // tree AND out of the tab order, or the subtopic buttons inside it would
    // still take focus. Once open, the height is the content's own — no cap
    // that could clip a long list of lessons.
    it('keeps a collapsed body inert and hidden, and opens it without a height cap', () => {
      render(<StudentLessonProgressModal {...defaultProps} />);

      const body = screen.getByTestId('accordion-content-topic-1');
      expect(body).toHaveAttribute('aria-hidden', 'true');
      expect(body).toHaveAttribute('inert');
      expect(body).toHaveClass('grid-rows-[0fr]');

      fireEvent.click(screen.getByText('Cinemática').closest('button')!);
      ...
      const subtopicBody = screen.getByTestId(
        'accordion-content-subtopic-subtopic-1-1'
      );
      expect(subtopicBody).toHaveAttribute('inert');
      fireEvent.click(screen.getByText('Aspectos iniciais').closest('button')!);
      expect(subtopicBody).not.toHaveAttribute('inert');
      expect(subtopicBody).toHaveClass('grid-rows-[1fr]');
    });
  ...
  describe('Sem exportação', () => {
    // O botão "Baixar relatório" foi removido deste modal em particular: o
    // relatório da página já exporta o mesmo recorte.
    it('não oferece download nem controles de impressão', () => {
      render(<StudentLessonProgressModal {...defaultProps} />);
