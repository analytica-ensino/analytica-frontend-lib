...
import { convertActivityFiltersToQuestionsFilter } from '../../utils/activityFiltersConverter';

/**
 * Maps API question type string to QUESTION_TYPE enum
 */
const mapQuestionTypeToEnum = (type: string | QUESTION_TYPE): QUESTION_TYPE => {
...
interface ActivityQuestionsListProps {
 ...
 */
export const ActivityQuestionsList = ({
  apiClient,
  onAddQuestion,
  onAddAutomatic,
}: ActivityQuestionsListProps) => {
  ...
  const getSubjectInfo = (question: Question) => {
    if (!question.knowledgeMatrix || question.knowledgeMatrix.length === 0) {
      return { assunto: 'Sem assunto', color: '#6B7280', iconName: 'BookOpen' };
    }
        ...
        {questions.map((question) => {
          ...
          return (
            <ActivityCardQuestionBanks
              ...
              iconName={subjectInfo.iconName}
              subjectColor={subjectInfo.color}
              isDark={isDark}
              assunto={subjectInfo.assunto}
              enunciado={question.statement}
              onAddToActivity={() => onAddQuestion?.(question)}
            />
          );
        })}
        {pagination?.hasNext && (
    ...
    <div className="w-full flex flex-col p-4 gap-2 overflow-hidden h-full min-h-0">
      <div className="flex flex-col gap-2 flex-shrink-0">
        ...
        <section className="flex flex-row justify-between items-center">
          ...
          {onAddAutomatic && (
            <Button size="small" onClick={onAddAutomatic}>
