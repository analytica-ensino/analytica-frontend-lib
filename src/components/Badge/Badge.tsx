...
export const Badge = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      {iconLeft && <span className={`${baseClassesIcon} ${sizeClassesIcon}`}>{iconLeft}</span>}
      {children}
      {iconRight && <span className={`${baseClassesIcon} ${sizeClassesIcon}`}>{iconRight}</span>}
    </div>
  );
};
