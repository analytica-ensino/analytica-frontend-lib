...
describe('Input', () => {
  ...
  describe('Search variant', () => {
    ...
    it('always shows magnifying glass icon on the left', () => {
      render(<Input variant="search" />);
      // The magnifying glass icon is rendered as a default left icon
      const leftIconWrapper = document.querySelector('.absolute.left-3');
    ...
    it('does not show clear button when disabled', () => {
      const TestComponent = () => {
        const [value] = React.useState('test text');
        return (
          <Input
            variant="search"
            value={value}
    ...
    it('does not show clear button when read-only', () => {
      const TestComponent = () => {
        const [value] = React.useState('test text');
        return (
          <Input
    ...
    it('applies responsive width classes', () => {
      ...
      const container = document.querySelector(
        '.w-full.max-w-lg.md\\:w-\\[488px\\]'
      );
      expect(container).toBeInTheDocument();
    });

    it('does not show helper text or error message for search variant', () => {
      render(
        <Input
          variant="search"
    ...
    it('applies correct padding for search variant', () => {
      render(<Input variant="search" data-testid="input" />);
      expect(screen.getByTestId('input')).toHaveClass('px-4');
    });

    it('applies correct padding with icons for search variant', () => {
      render(
        <Input
          variant="search"
          data-testid="input"
      ...
      // Should have pl-10 for left icon (magnifying glass) and pr-10 for right icon (clear button)
      expect(screen.getByTestId('input')).toHaveClass('pl-10', 'pr-10');
    });

    it('applies error state styles for search variant', () => {
      render(<Input variant="search" state="error" data-testid="input" />);
      expect(screen.getByTestId('input')).toHaveClass(
        'border-2',
        'rounded-full',
        'bg-[#F4FAFF]',
    ...
    it('applies disabled state styles for search variant', () => {
      render(<Input variant="search" disabled data-testid="input" />);
      expect(screen.getByTestId('input')).toHaveClass(
        'border-2',
        'rounded-full',
        'bg-[#F4FAFF]',
    ...
    it('applies read-only state styles for search variant', () => {
      render(<Input variant="search" readOnly data-testid="input" />);
      expect(screen.getByTestId('input')).toHaveClass(
        'border-2',
        'rounded-full',
        'bg-[#F4FAFF]',
    ...
    it('does not apply default background for search variant', () => {
      render(<Input variant="search" data-testid="input" />);
      expect(screen.getByTestId('input')).not.toHaveClass('bg-background');
    });

    it('applies zero vertical padding for search variant', () => {
      render(<Input variant="search" data-testid="input" />);
      expect(screen.getByTestId('input')).toHaveClass('py-0');
    });

    it('handles clear button with onChange but no onClear', () => {
      const handleChange = jest.fn();
      const TestComponent = () => {
        const [value, setValue] = React.useState('test');
        return (
    ...
    it('does not show clear button when value is empty string', () => {
      render(
        <Input
          variant="search"
          value=""
    ...
    it('handles all size variants with search', () => {
      const sizes = ['small', 'medium', 'large', 'extra-large'] as const;

      sizes.forEach((size) => {
        const { unmount } = render(
          <Input variant="search" size={size} data-testid={`input-${size}`} />
        );
        ...
        expect(screen.getByTestId(`input-${size}`)).toHaveClass(
          sizeClasses[size]
        );
        unmount();
      });
    ...
    it('renders with containerClassName alongside search classes', () => {
      render(<Input variant="search" containerClassName="custom-container" />);

      const container = document.querySelector(
        '.w-full.max-w-lg.md\\:w-\\[488px\\].custom-container'
      );
      expect(container).toBeInTheDocument();
    });

    it('handles placeholder correctly for search variant', () => {
      render(
        <Input
          variant="search"
