...
const ACTION_ICON_CLASSES = {
  warning: 'bg-success-300 text-text',
...
const CardActivesResults = forwardRef<HTMLDivElement, CardActivesResultsProps>(
  (
    ...
    const actionHeaderClasse = ACTION_HEADER_CLASSES[action];
    ...
    return (
      <>
        ...
        >
          ...
          >
            <span
              className={`size-7.5 rounded-full bg-warning-300 text-text flex items-center justify-center ${actionIconClasses}`}
            >
              {icon}
            </span>
          ...
          {extended && (
            <div className="flex flex-col items-center gap-2.5 pb-9.5 pt-2.5">
              <p
                className={`text-2xs font-medium uppercase ${actionHeaderClasse}`}
              >
                {header}
              </p>
